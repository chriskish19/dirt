...
core::codes core::backend::process::process_watch_entries()
{
    ...
    for (auto& entry : m_watch_entries_v) {
        ...
        ctx->src = entry.src_p;
        ctx->dst = entry.dst_p;
        ctx->di_set = new std::unordered_set(api::get_all_directories(entry.src_p));
...
core::codes core::backend::process::process_copy_entries()
{
    ...
    for (const auto& entry : m_copy_entries_v) {
        std::uintmax_t size = api::total_size(entry.src_p);
        auto found = std::find(entry.args_v.begin(), entry.args_v.end(),args::recursive);
        if (found != entry.args_v.end()) {
            
            auto start = std::chrono::high_resolution_clock::now();
            codes code = api::copy_directory_recursive(entry.dst_p, entry.src_p);
            ...
            auto start = std::chrono::high_resolution_clock::now();
            codes code = api::copy_directory_only(entry.dst_p, entry.src_p);
            auto end = std::chrono::high_resolution_clock::now();
    ...
    case file_action::copy:
    {
        ...
        try {
            ...
            if (copied == false and std::filesystem::exists(entry.dst_p) == false) {
                add(file_copy{ entry,file_copy_fail_pkg });
            }
#endif
        }
        catch (const std::filesystem::filesystem_error& e) {
    ...
    case file_action::delete_dst:
    {
        ...
        try {
            ...
            if (removed == false and std::filesystem::exists(entry.dst_p) == true) {
                add(file_delete{ entry,file_delete_fail_pkg });
            }
#endif
        }
        catch (const std::filesystem::filesystem_error& e) {
...
void core::backend::queue_system::directory(file_entry& entry)
{
    ...
    case file_action::delete_dst:
    {
        ...
        try {
            ...
            if (removed == 0) {
                add(directory_delete{ entry,directory_delete_fail_pkg });
            }
            else {
                std::string files_removed_message = "Removed files from: " + entry.dst_p.string() + '\n'
                    + "Total files removed: " + std::to_string(removed) + '\n';
                add(message{ files_removed_message });
...
void core::backend::queue_system::background_task(const file_entry& entry)
{
    ...
    case directory_completed_action::recursive_copy:
    {
        try {
            ...
            directory_info di;
            di.number_of_files = api::file_numbers(entry.dst_p);
            di.p = entry.dst_p;
...
bool core::backend::queue_system::skip_entry(file_entry& entry)
{
    ...
    for (const auto& part : entry.src_p) {
        ...
        if (std::filesystem::is_directory(partial_path) == true 
            ...
            {
                std::lock_guard<std::mutex> local_lock(m_set_mtx);
                directory_info di;
                di.p = partial_path;
...
void core::backend::background_queue_system::directory(file_entry& entry)
{
    ...
    case file_action::delete_dst:
    {
        ...
        if (removed == 0) {
            ...
            std::string removed_files_message = "Removed files from: " + entry.dst_p.string() + '\n'
                + "Total files removed: " + std::to_string(removed) + '\n';
            
            add(message{ removed_files_message });
